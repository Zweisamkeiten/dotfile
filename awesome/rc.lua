...
local net = lain.widget.net({
	settings = function()
		widget:set_markup(
			markup.font(
				widget_font,
				markup.font(widget_font, markup("#81c8be", ""))
	...
	{
		{
			widget = wibox.widget.textclock(
				'<span color="#e5c890" font="Nothing You Could Do 16"> %a %b %d, %H:%M </span>'
			),
		},
		--  bg = "blue",
		widget = wibox.container.background,
...
local modmap = {
	{
		"e",
		function()
			awful.util.spawn(terminal .. " -e " .. "zsh -c '$HOME/.local/bin/lvim'")
		end,
		"Editor",
	},
	{
		"g",
		function()
			awful.spawn.with_shell("gtkwave")
		end,
		"Gtkwave",
	},
	{
	...
	},
	{
		"q",
		function()
			awful.util.spawn(terminal .. " -e " .. "zsh -c 'emacsclient -nc'")
		end,
		"Emacs",
	},
	{
		"t",
		function()
			awful.util.spawn(terminal .. " -e " .. "tmux attach")
		end,
		"Tmux",
	},
	{
	...
	},
	{
		"s",
		function()
			awful.util.spawn([[zsh -c '~/c/b/mybin/shot']])
		end,
		"send shot to phone",
	},
	{
		...
		function()
			awful.util.spawn(
				terminal
					.. " -c videodir"
					.. " -e "
					.. "zsh -c 'cd $(xdg-user-dir VIDEOS) && ranger --choosedir=$XDG_STATE_HOME/ranger/rangerdir'"
			)
		end,
		"VideoDir",
	},
...
local clientkeys = gears.table.join(
	...
	awful.key({ modkey }, "[", function()
		awful.util.spawn(
			[[zsh -c 'xinput list | awk "/Touchpad/ {print \$6}" | awk -F "=" "{print \$2}" | xargs xinput --disable']]
		)
	end, { description = "Disable Touchpad", group = "user" }),
	awful.key({ modkey }, "]", function()
		awful.util.spawn(
			[[zsh -c 'xinput list | awk "/Touchpad/ {print \$6}" | awk -F "=" "{print \$2}" | xargs xinput --enable']]
		)
	end, { description = "Enable Touchpad", group = "user" }),
	awful.key({ modkey, "Shift" }, "a", function()
		awful.util.spawn("zsh -c 'maim -s $HOME/m/i/screenshot/shot$(date +%F_%H-%M-%S).png'")
	end, { description = "screenshot", group = "user" }),
	...
	end, { description = "lock screen", group = "user" })
)
...
awful.rules.rules = {
	...
	{ rule = { class = "wemeetapp" }, properties = { floating = true, ontop = true } },
	{ rule = { class = "QQ" }, properties = { floating = true, ontop = true } },
