...
local modmap = {
  ...
  { "v", function()
    awful.util.spawn(terminal ..
      " -c videodir" ..
      " -e " .. "zsh -c 'cd $(xdg-user-dir VIDEOS) && ranger --choosedir=$XDG_STATE_HOME/ranger/rangerdir'")
  end, "VideoDir" }
}
...
clientkeys = gears.table.join(
  ...
  awful.key({ modkey }, "[",
    function()
      awful.util.spawn([[zsh -c 'xinput list | awk "/Touchpad/ {print \$6}" | awk -F "=" "{print \$2}" | xargs xinput --disable']])
    end,
    { description = "Disable Touchpad", group = "user" }
  ),
  awful.key({ modkey }, "]",
    function()
      awful.util.spawn([[zsh -c 'xinput list | awk "/Touchpad/ {print \$6}" | awk -F "=" "{print \$2}" | xargs xinput --enable']])
    end,
    { description = "Enable Touchpad", group = "user" }
  ),
